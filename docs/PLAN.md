...
The deploy job is gated, as built (`msbuild.yml:330-333`), on `github.repository == 'ygordreyer/moonlight-xbox-plus' && github.event_name != 'pull_request' && (github.event_name != 'workflow_dispatch' || inputs.deploy == true)`, runs on `[self-hosted, xbox-lan]`, and carries `needs: build` so it consumes the uploaded artifact rather than rebuilding. Its own concurrency group `xbox-deploy` (`cancel-in-progress: false`) serializes deploys so two pushes in quick succession queue rather than race the console.
