...
PowerShell 7, 722 lines, already written and read as ground truth for this section. Actual parameters: `-ArtifactDir <path>` (mandatory); `-ConsoleArchitecture` (`x64` default, the Xbox CPU; `x86`, `arm`, `arm64` accepted); `-ConsoleAddress`, `-Username`, `-Password`, `-CredentialsPath` (default `$HOME/.xbox-deploy/credentials.json`); switches `-Require`, `-DryRun`, `-Launch`, `-AppUserModelId`, `-PullLocalState`, `-Screenshot`, `-RequireValidCertificate`; `-OutputDir`; timing `-InstallPollIntervalSec` (default 3), `-InstallPollTimeoutSec` (default 300), `-RequestTimeoutSec` (default 60).
...
3. `Resolve-DeployArtifact` searches `-ArtifactDir` recursively for `*.msixbundle`, falling back to `*.msix`, skips anything under a `Dependencies` folder, and takes the shallowest match: msbuild's `/p:AppxPackageDir` writes `<name>_<version>_Test\<name>_<version>_<arch>.msixbundle`, so the package sits one level below the artifact root, never at it (runs 34939401459 and 34970147629 failed here, the first on a root-only lookup, the second on a strict-mode `.Count` on a one-segment path). Dependencies come from the `Dependencies` folder beside the package, read the way `Add-AppDevPackage.ps1` in that same folder reads them: architecture-neutral `*.appx` and `*.msix` directly under it plus the one `<arch>` subfolder matching `-ConsoleArchitecture`; the other architecture folders are never uploaded.
